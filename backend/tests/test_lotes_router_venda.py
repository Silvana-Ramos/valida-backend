"""Testes de POST /lotes/{id_lote}/venda via TestClient.
...
def test_venda_parcial_retorna_201(client, db_mock):
    db_mock.get.return_value = _lote_confirmado("disponivel", Decimal("10"))

    resposta = client.post(
        f"/lotes/{ID_LOTE}/venda", json={"quantidade": 3}, headers=HEADER_VALIDO
    )
    ...
    assert corpo["tipo_movimentacao"] == "venda"
    db_mock.commit.assert_called_once()


def test_venda_total_retorna_201_e_saldo_zero(client, db_mock):
    db_mock.get.return_value = _lote_confirmado("disponivel", Decimal("5"))
...
def test_sem_header_retorna_401(client, db_mock):
    resposta = client.post(f"/lotes/{ID_LOTE}/venda", json={"quantidade": 3})
...
def test_lote_nao_confirmado_retorna_409(client, db_mock):
    db_mock.get.return_value = _lote(
        StatusLote.PENDENTE_CONFIRMACAO, "disponivel", Decimal("10")
    )
...
def test_saldo_insuficiente_retorna_409(client, db_mock):
    db_mock.get.return_value = _lote_confirmado("disponivel", Decimal("2"))

    resposta = client.post(
        f"/lotes/{ID_LOTE}/venda", json={"quantidade": 9}, headers=HEADER_VALIDO
    )
...
def test_id_mercado_em_query_string_e_ignorado(client, db_mock):
    ...
    resposta = client.post(
        f"/lotes/{ID_LOTE}/venda?id_mercado=999999",
        json={"quantidade": 3},
        headers=HEADER_VALIDO,
    )
